...
export default function AnalysisPage({ taskId }) {
  ...
  const [expandedLocationPosts, setExpandedLocationPosts] = useState(new Set());
  ...
  const [expandedSentimentPosts, setExpandedSentimentPosts] = useState(new Set());
  const [imageViewerOpen, setImageViewerOpen] = useState(false);
  const [selectedImage, setSelectedImage] = useState({ url: "", index: 0, allImages: [] });
  ...
  const [expandedDatePosts, setExpandedDatePosts] = useState(new Set());
  const [datePostsOffset, setDatePostsOffset] = useState(0);
  const [hasMoreDatePosts, setHasMoreDatePosts] = useState(true);
  const [loadingMoreDatePosts, setLoadingMoreDatePosts] = useState(false);
  ...
  const fetchDatePosts = async (date, taskId) => {
    ...
    setDatePosts([]);
    setDatePostsOffset(0);
    setHasMoreDatePosts(true);
    ...
    try {
      ...
      // Query posts using post-counts endpoint with date filter
      try {
        const res = await axios.get(`${API_BASE}/tasks/${taskId}/posts-by-date/`, {
          params: {
        ...
        if (res.data?.results && res.data.results.length > 0) {
          ...
          // Check if there are more posts
          const total = res.data?.total || res.data?.total_count || 0;
          setHasMoreDatePosts(posts.length === limit && (total === 0 || posts.length < total));
          setDatePostsOffset(posts.length);
        } else if (res.data?.posts && res.data.posts.length > 0) {
          posts = res.data.posts;
          setHasMoreDatePosts(posts.length === limit);
          setDatePostsOffset(posts.length);
          ...
          setHasMoreDatePosts(false);
        }
      } catch (err) {
        console.log("Error fetching date posts:", err.message);
        setHasMoreDatePosts(false);
      }
  ...
  // Load more date posts
  const loadMoreDatePosts = async () => {
    if (!selectedDate || !taskId || loadingMoreDatePosts || !hasMoreDatePosts) return;

    setLoadingMoreDatePosts(true);
    try {
      const limit = 3;
      ...
      if (newPosts.length > 0) {
        setDatePosts((prev) => [...prev, ...newPosts]);
        setDatePostsOffset((prev) => prev + newPosts.length);

        // Check if there are more posts
        const total = res.data?.total || res.data?.total_count || 0;
        setHasMoreDatePosts(
          newPosts.length === limit && (total === 0 || datePosts.length + newPosts.length < total)
        );
      } else {
        setHasMoreDatePosts(false);
      }
    } catch (err) {
      console.error("Error loading more date posts:", err);
      setHasMoreDatePosts(false);
    } finally {
      setLoadingMoreDatePosts(false);
    }
  };
    ...
    <MDBox mt={2}>
      ...
      <Dialog
        ...
        onClose={() => {
          ...
          setLocationPostsOffset(0);
          setHasMoreLocationPosts(true);
          setExpandedLocationPosts(new Set());
        }}
        maxWidth="md"
        fullWidth
        PaperProps={{
              ...
              {locationPosts.map((post, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {postText &&
                      (() => {
                        const postId = post.id || post.content_id || i;
                        const isExpanded = expandedLocationPosts.has(postId);
                        const textLimit = 200;
                        const isLongText = postText.length > textLimit;
                        const displayText =
                          isLongText && !isExpanded
                            ? postText.substring(0, textLimit) + "..."
                            : postText;

                        return (
                          <MDBox sx={{ mb: postPhotos.length > 0 ? 1.5 : 0 }}>
                            <MDTypography variant="body2" sx={{ color: "#333", lineHeight: 1.6 }}>
                              {displayText}
                            </MDTypography>
                            {isLongText && (
                              <MDTypography
      ...
      <Dialog
        ...
        onClose={() => {
          ...
          setSentimentPostsOffset(0);
          setHasMoreSentimentPosts(true);
          setExpandedSentimentPosts(new Set());
        }}
        maxWidth="md"
        fullWidth
        PaperProps={{
                ...
                return (
                  ...
                  >
                    ...
                    {postText &&
                      (() => {
                        const postId = post.id || post.content_id || i;
                        const isExpanded = expandedSentimentPosts.has(postId);
                        const textLimit = 200;
                        const isLongText = postText.length > textLimit;
                        const displayText =
                          isLongText && !isExpanded
                          ...
                          <MDBox sx={{ mb: postPhotos.length > 0 ? 1.5 : 0 }}>
                            ...
                            {isLongText && (
                              <MDTypography
                                variant="caption"
                                onClick={() => {
                                  setExpandedSentimentPosts((prev) => {
                                    const newSet = new Set(prev);
                                    if (isExpanded) {
                                      newSet.delete(postId);
                                    } else {
                                ...
                                sx={{
                                  ...
                                  display: "inline-block",
                                  "&:hover": {
                                    color: alpha(sentimentColor, 0.8),
                                    textDecoration: "underline",
                                  },
                                }}
                              >
      ...
      >
        ...
        >
          <MDTypography variant="h6" sx={{ color: "#fff" }}>
            ...
            {selectedImage.allImages.length > 0
              ? `${selectedImage.index + 1} of ${selectedImage.allImages.length}`
              : ""}
          </MDTypography>
          <IconButton
        ...
        >
          {selectedImage.url && (
            <>
              {/* Previous Button */}
              {selectedImage.allImages.length > 1 && selectedImage.index > 0 && (
                <IconButton
                  onClick={() => {
                    const prevIndex = selectedImage.index - 1;
                    setSelectedImage({
                      ...selectedImage,
                      url: selectedImage.allImages[prevIndex],
                      index: prevIndex,
                    });
                  }}
                  sx={{
              ...
              {selectedImage.allImages.length > 1 &&
                selectedImage.index < selectedImage.allImages.length - 1 && (
                  <IconButton
                    onClick={() => {
                      const nextIndex = selectedImage.index + 1;
                      setSelectedImage({
                        ...selectedImage,
                        url: selectedImage.allImages[nextIndex],
                        index: nextIndex,
                      });
                    }}
                    sx={{
      ...
      >
        ...
        >
                ...
                return (
                  ...
                  >
                    ...
                    {postText &&
                      (() => {
                        const postId = post.id || post.content_id || i;
                        const isExpanded = expandedDatePosts.has(postId);
                        const textLimit = 200;
                        const isLongText = postText.length > textLimit;
                        const displayText =
                          isLongText && !isExpanded
          ...
          {/* Load More Button */}
          {!loadingDatePosts && datePosts.length > 0 && hasMoreDatePosts && (
            <MDBox
              sx={{
                borderTop: `1px solid #E4E6EB`,
                pt: 2,
